...
# Optimized prime generation using Crypto.Util.number
def generate_p_q(k):
    """Generate parameters p, q, g where q is k-bit prime and q|(p-1)"""
    # Generate k-bit prime q
    q = getPrime(k)
    
    # Find p = 2*q + 1 (safe prime) - faster than searching for arbitrary t
    p = 2 * q + 1
    while not isPrime(p):
        q = getPrime(k)
        p = 2 * q + 1
...
def signcrypt(param, sk_S, pk_R, m):
    ...
    # Compute r = H(m || bind || K)
    K_bytes = K.to_bytes(elem_len, 'big')
    r = H(m + bind + K_bytes, q)
...
def unsigncrypt(param, pk_S, sk_R, C):
    ...
    bind = y_S.to_bytes(elem_len, 'big') + y_R.to_bytes(elem_len, 'big')
    K_bytes = K.to_bytes(elem_len, 'big')
    expected_r = H(m + bind + K_bytes, q)
    
    return m if expected_r == r else None

def benchmark(k=128, iterations=100, msg_sizes=None):
    """Benchmark the signcryption scheme"""
    if msg_sizes is None:
        msg_sizes = [64, 256, 1024, 4096]  # bytes
    
    print(f"\n{'='*60}")
    print(f"Zheng Signcryption Benchmark (Security parameter: {k}-bit)")
    print(f"{'='*60}")
    ...
    avg_1kb_signcrypt = [t for t in signcrypt_times if len(signcrypt_times) > 0][-1] * 1000  # Last 1KB measurement
