...
from lib.crypt import create_key
...
def keys():
  ...
  if data is None:
     return jsonify({"message": "No data received", "code": NO_DATA_CODE})
  
  if data.get("public_key") is None or data.get("common_public_key") is None or data.get("chameleon_hash") is None:
     return jsonify({"message": "Field is missing", "code": FIELD_MISSING_CODE})
